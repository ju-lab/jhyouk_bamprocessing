...
bwa mem -M -t $thr -R "@RG\tID:$sampleName\tLB:1\tSM:$sampleName\tPL:ILLUMINA" $reference $fastq1 $fastq2 > $sampleName.sam 2> $sampleName.bwamem.out 

echo sam to bam
samtools view -Sb -@ $((thr-1)) -o $sampleName.bam $sampleName.sam > $sampleName.StoB.out 2>&1 
rm $sampleName.sam 

echo sort
samtools sort -@ $((thr-1)) -o $sampleName.s.bam $sampleName.bam > $sampleName.sort.out 2>&1 
rm $sampleName.bam 
...
echo recalibration
java -Xms8g -Xmx12g -jar /home/users/tools/gatk/gatk-3.5/GenomeAnalysisTK.jar -T BaseRecalibrator -R /home/users/jhyouk/99_reference/mouse/mm10/GRCm38.fa -I $sampleName.s.md.ir.bam --knownSites /home/users/jhyouk/99_reference/mouse/mm10/mgp.v5.merged.indels.dbSNP142.normed.reduce.s.vcf -knownSites /home/users/jhyouk/99_reference/mouse/mm10/mgp.v5.merged.snps_all.dbSNP142.reduce.s.vcf -o $sampleName.s.md.ir.bam.table -nct $thr > $sampleName.BR.out 2>&1 
java -Xms8g -Xmx12g -jar /home/users/tools/gatk/gatk-3.5/GenomeAnalysisTK.jar -T PrintReads -R /home/users/jhyouk/99_reference/mouse/mm10/GRCm38.fa -I $sampleName.s.md.ir.bam -BQSR $sampleName.s.md.ir.bam.table -o $sampleName.s.md.ir.br.bam -nct $thr > $sampleName.PR.out 2>&1 
